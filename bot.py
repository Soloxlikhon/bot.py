...
def create_payment_config_table():
    ...
    try:
        ...
        conn.commit()
        print("✅ Payment config table initialized")
    except Exception as e:
        print(f"⚠️ Payment config table error: {e}")
    finally:
        cursor.close()
        conn.close()
...
def create_proxy_table():
    ...
    cursor = conn.cursor()
    try:
        # Users table এ balance column যোগ করুন
        try:
            cursor.execute("ALTER TABLE users ADD COLUMN balance FLOAT DEFAULT 0.0")
        except:
            pass
        
        # proxy_brands table তৈরি করুন - সঠিক কলাম সহ
        cursor.execute("""
        ...
        # যদি price_usd কলাম না থাকে তাহলে যোগ করুন
        try:
            cursor.execute("ALTER TABLE proxy_brands ADD COLUMN price_usd FLOAT DEFAULT 0.0")
        except:
            pass
        
        # যদি rate_bdt কলাম না থাকে তাহলে যোগ করুন
        try:
            cursor.execute("ALTER TABLE proxy_brands ADD COLUMN rate_bdt FLOAT DEFAULT 0.0")
        except:
            pass
        
        # proxy_stock table তৈরি করুন
        cursor.execute("""
        ...
        # pending_deposits table তৈরি করুন
        cursor.execute("""
        ...
        conn.commit()
        print("✅ Proxy tables initialized successfully")
    except Exception as e:
        print(f"⚠️ Proxy table error: {e}")
    finally:
        cursor.close()
        conn.close()
...
def validate_payment_number(service, number):
    if service != "Binance":
        ...
        if len(digits_only) < 11:
            return False, f"❌ Minimum 11 digits required. You provided: {len(digits_only)} digits"
    return True, "Valid"
...
def save_proxy_name(message):
    ...
    if brand_name == "🔙 Back to User Panel": 
        return
    conn = connection_pool.get_connection()
    cursor = conn.cursor()
    try:
...
def validate_price_input(price_str):
    try:
        numbers = re.findall(r"\d+\.?\d*", price_str.strip())
        
        if not numbers:
            return None, "❌ কোনো বৈধ সংখ্যা পাওয়া যায়নি!"
        
        price = float(numbers[0])
        
        if price <= 0:
            return None, "❌ মূল্য শূন্য বা নেতিবাচক হতে পারে না!"
        
        return price, "Valid"
        
    except ValueError:
        return None, "❌ অবৈধ সংখ্যা ফরম্যাট!"

def process_price_input(message, brand):
    price_str = message.text.strip()
    
    if price_str == "🔙 Back to User Panel":
        return
    
    price_bdt, validation_msg = validate_price_input(price_str)
    
    if price_bdt is None:
        bot.send_message(message.chat.id, validation_msg)
        msg = bot.send_message(message.chat.id, 
            f"💰 **{brand}-এর মূল্য আবার সেট করুন:**\n\n"
            f"📝 শুধুমাত্র সংখ্যা লিখুন (উদাহরণ: 254)")
    ...
    preview_text = (
        ...
        f"━━━━━━━━━━━━━━━━━━\n\n"
        f"🏷 **ব্র্যান্ড:** `{brand}`\n"
        f"৳ **BDT মূল্য:** `{price_bdt:.2f} টাকা`\n"
        f"💵 **USD মূল্য:** `${price_usd:.2f}`\n\n"
        f"📊 **এক্সচেঞ্জ রেট:** `1 USD = {exchange_rate} টাকা`\n\n"
        f"✅ সঠিক হলে 'Confirm' বাটন চাপুন।"
    )
    
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("✅ Confirm", callback_data=f"confirm_price_{brand}_{price_bdt}_{price_usd}"),
        types.InlineKeyboardButton("❌ Edit", callback_data=f"edit_price_{brand}")
    )
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("auto_") and not call.data.startswith("auto_pay_"))
...
def handle_add_payment(call):
    ...
    if service == "Binance":
        ...
        text = f"📱 **{service} নম্বর দিন:** (Minimum 11 digits)"
...
def save_payment_number(message, service):
    ...
    if not is_valid:
        bot.send_message(
            ...
            f"❌ **ভুল ফরম্যাট!**\n\n"
            f"{validation_msg}\n\n"
            f"সঠিক নম্বর দিয়ে আবার চেষ্টা করুন।",
            parse_mode="Markdown"
        )
        msg = bot.send_message(message.chat.id, f"📱 **{service} নম্বর দিন:** (Minimum 11 digits)")
        bot.register_next_step_handler(msg, save_payment_number, service)
        return
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("confirm_price_"))
def confirm_price(call):
    try:
        data = call.data.split("_")
        brand = data[2]
        price_bdt = float(data[3])
        price_usd = float(data[4])
        
        conn = connection_pool.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute(
                "UPDATE proxy_brands SET price_usd = %s, rate_bdt = %s WHERE brand_name = %s",
                (price_usd, price_bdt, brand)
            )
            conn.commit()
            
            bot.answer_callback_query(call.id, f"✅ মূল্য সংরক্ষিত হয়েছে!", show_alert=True)
            
            success_text = (
                f"✅ **মূল্য আপডেট হয়েছে!**\n\n"
                f"🏷 **ব্র্যান্ড:** `{brand}`\n"
                f"৳ **BDT:** `{price_bdt:.2f} টাকা`\n"
                f"💵 **USD:** `${price_usd:.2f}`\n\n"
                f"💾 সফলভাবে ডাটাবেসে সংরক্ষিত হয়েছে।"
            )
            
            bot.edit_message_text(success_text, call.message.chat.id, call.message.message_id, parse_mode="Markdown")
            bot.send_message(call.message.chat.id, "**Admin Panel**", reply_markup=admin_panel_menu())
...
def edit_price(call):
    brand = call.data.replace("edit_price_", "")
    bot.answer_callback_query(call.id)
    ...
    try:
        ...
        if payment_data.get("status") == "COMPLETED":
            ...
            try:
                ...
                new_balance_bdt = float(result[0]) if result else 0.0
...
def start_broadcasting(message):
    ...
    if broadcast_text == "🔙 Back to User Panel": 
        return
...
def start(message):
    ...
    if is_member(uid):
        welcome_text = (
            f"👋 স্বাগতম!\n\n"
            f"🎯 **আমাদের সার্ভিস:**\n"
            f"🫂 মার্কেট এর সবচাইতে কম রেটে GB Proxy 💯\n"
            f"✅ ট্রাস্টেড সেলার\n"
            f"🤖 প্রক্সি চেক বট\n"
            f"💥 ২৪/৭ সাপোর্ট\n\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"✅ আপনার অ্যাকাউন্ট সক্রিয়।\n"
            f"নীচের মেনু থেকে শুরু করুন! 👇"
        )
        bot.send_message(message.chat.id, welcome_text, reply_markup=main_menu(uid), parse_mode="Markdown")
    else:
        markup = types.InlineKeyboardMarkup()
        ...
        not_member_text = (
            f"👋 স্বাগতম!\n\n"
            f"❌ **চ্যানেলে যোগ দিন প্রথমে!**\n\n"
            f"🫂 আমাদের বট এ মার্কেট এর সবচাইতে কম রেটে GB Proxy পাবেন 💯\n"
            f"ট্রাস্টেড এর সাথে সেল করি ✅\n"
            f"সাথে পাবেন যে কোন প্রক্সি এমবি চেক করার জন্য বট 🤖\n"
            f"যেকোনো সমস্যা হলে সাপোর্টে যোগাযোগ করুন সাথে সাথে 💥\n\n"
            f"📢 নীচের বাটনে ক্লিক করে চ্যানেলে যোগ দিন!"
        )
        bot.send_message(message.chat.id, not_member_text, reply_markup=markup, parse_mode="Markdown")
...
def process_buy_proxy(call):
    ...
    try:
        cursor.execute("SELECT balance FROM users WHERE user_id = %s", (user_id,))
        result = cursor.fetchone()
        user_balance = float(result[0]) if result and result[0] else 0.0
        
        # প্রক্সির মূল্য পান
        price_usd, price_bdt = get_proxy_pricing(brand)
        
        # ব্যালেন্স চেক করুন
        if user_balance < price_bdt:
            required_amount = price_bdt - user_balance
            required_usd = required_amount / EXCHANGE_RATE
            
            error_text = (
                f"❌ **অপর্যাপ্ত ব্যালেন্স!**\n\n"
                f"💰 **প্রয়োজন:** `{price_bdt:.2f} টাকা` (${price_usd:.2f})\n"
                f"💳 **আপনার ব্যালেন্স:** `{user_balance:.2f} টাকা`\n"
                f"❌ **ঘাটতি:** `{required_amount:.2f} টাকা` (${required_usd:.2f})\n\n"
                f"⬇️ প্রথমে ডিপোজিট করুন!"
            )
            
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("💳 Deposit করুন", callback_data="dep_manual"))
            markup.add(types.InlineKeyboardButton("❌ Cancel", callback_data="cancel_buy"))
            
            bot.answer_callback_query(call.id)
            bot.send_message(call.message.chat.id, error_text, reply_markup=markup, parse_mode="Markdown")
            return
        
        # ব্যালেন্স সফিশিয়েন্ট - প্রক্সি পান
        ...
        cursor.execute("UPDATE users SET balance = balance - %s WHERE user_id = %s", (price_bdt, user_id))
        conn.commit()
        
        # সফল মেসেজ
        new_balance_bdt = user_balance - price_bdt
        new_balance_usd = new_balance_bdt / EXCHANGE_RATE
        ...
        success_text = (
            ...
            f"🔑 **Proxy:**\n"
            f"`{proxy}`\n\n"
            f"💳 **টাকা কাটা হয়েছে:** `{price_bdt:.2f} টাকা`\n"
            f"💰 **নতুন ব্যালেন্স:** `{new_balance_bdt:.2f} টাকা` (${new_balance_usd:.2f})"
        )
...
def proxy_management_callback(call):
    ...
    bot.edit_message_text(
        text=f"🛠 **Management for: {brand_name}**",
...
def management_actions(call):
    ...
    elif data.startswith("add_price_"):
        brand = data.replace("add_price_", "")
        msg = bot.send_message(
            chat_id,
            f"💰 **{brand}-এর মূল্য সেট করুন**\n\n"
            f"📝 শুধুমাত্র সংখ্যা লিখুন (উদাহরণ: 254)\n\n"
            f"💵 এক্সচেঞ্জ রেট: 1 USD = {EXCHANGE_RATE} টাকা\n"
            f"📊 USD স্বয়ংক্রিয়ভাবে গণনা করা হবে"
        )
        bot.register_next_step_handler(msg, process_price_input, brand)

    elif data.startswith("edit_price_"):
        brand = data.replace("edit_price_", "")
        msg = bot.send_message(
            chat_id,
            f"💰 **{brand}-এর মূল্য সম্পাদন করুন**\n\n"
            f"📝 শুধুমাত্র সংখ্যা লিখুন (উদাহরণ: 254)\n\n"
            f"💵 এক্সচেঞ্জ রেট: 1 USD = {EXCHANGE_RATE} টাকা"
        )
        bot.register_next_step_handler(msg, process_price_input, brand)
        ...
        try:
            ...
            if not brands:
                ...
                for b in brands:
                    ...
                    if stock_count > 0:
                        btn_text = f"✅ {brand_name} (${price_usd:.2f}/{int(rate_bdt)} TK) - 📦 {stock_count}"
                        markup.add(types.InlineKeyboardButton(btn_text, callback_data=f"buy_{brand_name}"))
                    else:
                        btn_text = f"❌ {brand_name} - Out of Stock"
...
def out_of_stock_handler(call):
    bot.answer_callback_query(call.id, "❌ এই প্রক্সি স্টকে নেই!", show_alert=True)
